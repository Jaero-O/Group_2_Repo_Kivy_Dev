...
def analyze_image(image_path: str) -> dict:
    ...
    # Perform prediction (either real or mocked)
    disease_name, confidence = predictor.predict(image_path)
    
    # Calculate severity percentage and name based on the prediction
    if disease_name == "Healthy":
        severity_percentage = 0.0
        severity_name = "Healthy"
    else:
        severity_percentage = calculate_severity_percentage(image_path)
        # Determine severity name based on the calculated percentage
