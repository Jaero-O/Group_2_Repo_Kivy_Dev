...
class DatabaseManager:
    def __init__(self, db_path):
        ...
        # Ensure WAL mode is set once for the database file
        self._init_wal_mode()

    def _init_wal_mode(self):
        """Ensures the database is in WAL mode (persistent)."""
        conn = None
        try:
            conn = sqlite3.connect(self.db_path)
            conn.execute("PRAGMA journal_mode=WAL;")
            conn.commit() # Commit the PRAGMA change
            print(f"Database '{self.db_path}' set to WAL mode.")
    ...
    def _create_connection(self):
        """
        Creates a new, optimized connection for the calling thread.
        This is the *only* method that should call sqlite3.connect.
        """
        conn = None
        try:
